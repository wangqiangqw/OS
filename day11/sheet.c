...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1,int h0)
{
    ...
    if(vx1>=ctl->xsize) vx1 = ctl->xsize-1;
    if(vy1>=ctl->ysize) vy1 = ctl->ysize-1;
    unsigned char *buf, c, *vram = ctl->vram;
    struct SHEET *sht;
    for (h = h0>0?h0:0; h <= ctl->top; h++) {
        sht = ctl->sheets[h];
        buf = sht->buf;
        bx0=vx0 - sht->vx0;
        bx0 = bx0>=0? bx0:0;
        bx1=vx1 - sht->vx0;
        bx1 = bx1<=(sht->bxsize-1)? bx1:sht->bxsize-1;
        by0=vy0 - sht->vy0;
        by0 = by0>=0? by0:0;
        by1=vy1 - sht->vy0;
...
void sheet_refreshfull(struct SHTCTL *ctl)
{
    int h, bx, by, vx, vy;
    unsigned char *buf, c, *vram = ctl->vram;
    struct SHEET *sht;
